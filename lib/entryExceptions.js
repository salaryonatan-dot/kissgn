...
// Two-tier per-date operations-ledger limits (NO pruning / rollover / compaction).
//   A valid replay is honored regardless of count (checked before these limits).
//   manager soft limit: at >=32, only owner/super_owner may create new operations.
//   owner hard limit:   at >=128, no new operation is permitted for anyone.
export const MANAGER_OPERATION_SOFT_LIMIT = 32;
export const OWNER_OPERATION_HARD_LIMIT = 128;
...
export function applyOperation(currentEnvelope, cmd) {
  ...
  // Two-tier ledger limits (fail-closed, no pruning). Replay already returned above,
  //   so a valid replay ALWAYS succeeds regardless of count. actorRole is the trusted
  //   SERVER role (never a client-supplied value).
  const opCount = Object.keys(ops).length;
  if (opCount >= OWNER_OPERATION_HARD_LIMIT) {
    return { outcome: "conflict", code: "operation_hard_limit_reached" };
  }
  if (opCount >= MANAGER_OPERATION_SOFT_LIMIT && !(cmd.actorRole === "owner" || cmd.actorRole === "super_owner")) {
    return { outcome: "conflict", code: "operation_owner_required" };
  }
