...
// ─── RBAC ─────────────────────────────────────────────────────────────────────
// super_owner is a network-level super-privilege ABOVE owner. It is intentionally
// NOT in VALID_ROLES: VALID_ROLES gates which roles may be *assigned* to sub-users
// (see api/admin.js), and super_owner must never be assignable that way. Access is
// still granted only after the tenant membership check in requireTenantAccess.
const ROLE_RANK = { super_owner: 5, owner: 4, manager: 3, shift_manager: 2, viewer: 1 };
...
export async function requireTenantAccess(uid, tenantId, minRole) {
  ...
  // Accept any role that has a defined numeric rank — this recognizes super_owner
  // (rank 5, above owner) as well as the four sub-roles, while still rejecting
  // unknown/unset roles and prototype keys (e.g. "constructor"). super_owner is
  // accepted here WITHOUT being in VALID_ROLES, so it remains non-assignable.
  if (typeof role !== "string" || typeof ROLE_RANK[role] !== "number") throw { status: 403, msg: "no valid role assigned" };
