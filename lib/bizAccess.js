...
/**
 * Flatten every `= true` biz_access path across a `tenants/` snapshot value
 * into a Set of full RTDB paths (tenants/{tid}/biz_access/{bizId}/{uid}).
 * Used by the migration's reconciliation/cleanup to find UNEXPECTED entries.
 */
export function flattenBizAccess(tenantsData) {
  const out = new Set();
  const tenants = (tenantsData && typeof tenantsData === "object") ? tenantsData : {};
  for (const [tid, tv] of Object.entries(tenants)) {
    const ba = tv && tv.biz_access;
    if (ba && typeof ba === "object") {
      for (const [bizId, membersObj] of Object.entries(ba)) {
        if (membersObj && typeof membersObj === "object") {
          for (const [uid, val] of Object.entries(membersObj)) {
            if (val === true) out.add(`tenants/${tid}/biz_access/${bizId}/${uid}`);
          }
        }
      }
    }
